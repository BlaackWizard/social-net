...
from src.auth.adapters.email_sender.confirmation_token_sender import ConfirmationTokenSender
...
class InteractorProvider(Provider):
    ...
    confirmation_token_processor = provide(ConfirmationTokenProcessor, scope=Scope.REQUEST)
    password_hasher = provide(Argon2PasswordHasher, provides=PasswordHasher, scope=Scope.REQUEST)
    token_bearer_parser = provide(TokenBearerIdProvider, scope=Scope.REQUEST)
    token_sender = provide(ConfirmationTokenSender, provides=TokenSender, scope=Scope.REQUEST)
